...
def train_one_epoch_ssl(model, optimizer, train_loader, model_func, lr_scheduler, momentum_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        # moving average update of target
        with torch.no_grad():
            m = next(momentum_scheduler)
            if dist_train:
                for param_q, param_k in zip(model.module.backbone_3d.encoder.parameters(), model.module.backbone_3d.target_encoder.parameters()):
                    param_k.data.mul_(m).add_((1.-m) * param_q.detach().data)
            else:
                for param_q, param_k in zip(model.backbone_3d.encoder.parameters(), model.backbone_3d.target_encoder.parameters()):
                    param_k.data.mul_(m).add_((1.-m) * param_q.detach().data)
...
def train_model_ssl(model, optimizer, train_loader, model_func, lr_scheduler, momentum_scheduler, optim_cfg,
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            accumulated_iter = train_one_epoch_ssl(
...
def train_model_with_eval(model, optimizer, train_loader, test_loader, model_func, lr_scheduler, optim_cfg,
                start_epoch, total_epochs, start_iter, rank, tb_log, ckpt_save_dir, train_sampler=None,
                lr_warmup_scheduler=None, ckpt_save_interval=1, max_ckpt_save_num=50,
                merge_all_iters_to_one_epoch=False, logger=None, dist_test=False, eval_output_dir=None):
    accumulated_iter = start_iter
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        total_it_each_epoch = len(train_loader)
        if merge_all_iters_to_one_epoch:
        ...
        for cur_epoch in tbar:
            ...
            if (cur_epoch+1) % 5 == 0:
                with torch.no_grad():
                    logger.info('**********************Start evaluation epoch {}**********************'.format(cur_epoch+1))
                    val_tb_dict = eval_utils.eval_one_epoch(
                        cfg, model, test_loader, cur_epoch+1, logger, dist_test=dist_test,
