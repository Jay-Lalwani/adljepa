...
We tested whether a self-supervised AI encoder (one that learns by predicting missing information rather than relying on manual labels) can improve object detection using only radar data. We built upon AD-L-JEPA, an architecture originally proposed for LiDAR point clouds. Its core mechanism works by artificially masking out parts of a bird's-eye view map and forcing the model to predict the hidden features based on the surrounding visible context. Our core contribution was adapting this spatial-prediction concept to the highly sparse and noisy domain of automotive radar, and integrating it into a full end-to-end detection pipeline to measure its practical performance.
