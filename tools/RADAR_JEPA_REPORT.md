...
The controlled comparison used the same supervised dataset, detector config, training schedule, and validation split. The strongest JEPA run improved the most important radar behavior: it produced fewer false positives while keeping recall essentially unchanged. It did not improve every metric, especially BEV AP and velocity error, so we can prelimenarily conclude that radar JEPA helped false-positive suppression and localization quality in this setting, not that it universally improved detection.
